...
function init() {
    ...
    */
    ...
    document.querySelector('#score').innerText = 'Текущий счет: 0';
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        document.querySelector('#score').innerText = 'Текущий счет: '+ ++score;
    }
    return check;
}
...
function haveObstacle(unit) {
    ...
    // Если еда
    if (unit_classes.includes('obs-unit')) {
        check = true;
...
function createObstacle() {
    ...
    while (!obstacleCreated) { //пока еду не создали
        // рандом
        var obs_x = Math.floor(Math.random() * FIELD_SIZE_X);
        var obs_y = Math.floor(Math.random() * FIELD_SIZE_Y);

        var obs_cell = document.getElementsByClassName('cell-' + obs_y + '-' + obs_x)[0];
        var obs_cell_classes = obs_cell.getAttribute('class').split(' ');

        // проверка на змейку
        if (!obs_cell_classes.includes('snake-unit')) {
            var classes = '';
            for (var i = 0; i < obs_cell_classes.length; i++) {
                classes += obs_cell_classes[i] + ' ';
            }

            obs_cell.setAttribute('class', classes + 'obs-unit');
